...
+ (BOOL)installIPA:(NSURL *)ipaURL {
    NSLog(@"[DEBUG] Iniciando installIPA com: %@", ipaURL.path);
    
    // 1. Criar diretório
    NSString *appFolder = @"~/Library/Trollstore/Apps";
    NSError *error = nil;
    if (![[NSFileManager defaultManager] createDirectoryAtPath:appFolder
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"[ERRO] Falha ao criar pasta: %@", error);
        return NO;
    }
    NSLog(@"[DEBUG] Pasta criada: %@", appFolder);
    
    // 2. Descompactar via unzip (usando runCommand)
    BOOL unzipSuccess = [self runCommand:@"/usr/bin/unzip"
                                withArgs:@[ipaURL.path, @"-d", appFolder]];
    if (!unzipSuccess) {
        NSLog(@"[ERRO] unzip falhou para %@", ipaURL.path);
        return NO;
    }
    NSLog(@"[DEBUG] unzip concluído.");
    
    // 3. Listar conteúdo da pasta
    NSArray *contents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:appFolder error:&error];
    if (!contents) {
        NSLog(@"[ERRO] Não foi possível listar %@: %@", appFolder, error);
        return NO;
    }
    NSLog(@"[DEBUG] Conteúdo da pasta: %@", contents);
    ...
    if (!appBundle) {
        NSLog(@"[ERRO] Nenhum .app encontrado em %@", appFolder);
        return NO;
    }
    NSLog(@"[DEBUG] App bundle encontrado: %@", appBundle);
    
    // 5. Aplicar a race condition (que provavelmente falha)
    [self triggerLaunchdRaceForApp:appBundle];
    NSLog(@"[DEBUG] Race condition aplicada.");
